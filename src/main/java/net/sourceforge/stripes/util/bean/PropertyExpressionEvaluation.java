...
/**
 * The dynamic partner to a PropertyExpression that represents the evaluation of the expression against a particular
 * bean or starting object.  When constructed the evaluation will examine type information on the bean and nested
 * properties to create a chain of type information for the expression.  The evaluation can then be used (repeatedly) to
 * determine the type of the expression, retrieve it's value and set it's value - all against the supplied object.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public class PropertyExpressionEvaluation {
    ...
    /**
     * Constructs a new PropertyExpressionEvaluation for the expression and bean supplied. Loops through the expression
     * creating NodeEvaluation objects corresponding to each node in the expression and then fills in type information
     * so that it is accessible to all further calls.
     *
    ...
    /**
     * Examines the expression in context of the root bean provided to determine type information for each node.  Does
     * this by traversing a node at a time and examining the various sources of type information available.
     */
    ...
    /**
     * Fetches the type of a property with the given name on the Class of the specified type. Uses the methods first to
     * fetch the generic type if a PropertyDescriptor can be found, otherwise looks for a public field and returns its
     * generic type.
     *
    ...
    /**
     * <p>Locates and returns a non-bridge method for the method supplied. In certain cases the Introspector will return
     * PropertyDescriptors that contain bridge methods for read and write methods. This usually results from classes
     * implementing generic interfaces that contain accessor method specifications with type parameters. Since the
     * bridge methods have inappropriate/unhelpful return and parameter types it is necessary to locate the non-bridge
     * method and use that instead.</p>
     * <p/>
     * <p>When supplied with a non-bridge method, the method parameter passed in is returned immediately and no other
     * work is performed.</p>
     *
    ...
    /**
     * <p>Determines the type of the supplied node and sets appropriate information on the node. The type is discovered
     * by fetching (and instantiating if necessary) all prior values in the expression to determine the actual type of
     * the prior node.  The prior node is then examined to determine the type of the node provided.</p>
     * <p/>
     * <p>After this method executes either 1) all necessary type information will be set on the node and the
     * appropriate type object returned or 2) an exception will be thrown.</p>
     *
     * @param end the node to instantiate up to and determine the type of
     * @return the Type of the node if possible
     * @throws NoSuchPropertyException if the previous node is a JavaBean (i.e. non-collection) node and does not
     *                                 contain a property with the corresponding name
     * @throws EvaluationException     if the previous node is a List or Map and does not contain enough information to
     *                                 determine the type
     */
    ...
    /**
     * Attempts to convert the {@link Type} object into a Class object. Currently will extract the raw type from a
     * {@link ParameterizedType} and the appropriate bound from a {@link WildcardType}. If the result after these
     * operations is a Class object it will be cast and returned. Otherwise will return null.
     *
    ...
    /**
     * Scans backwards in the expression for the last node which contained a JavaBean type and attempts to use the type
     * arguments to that class to find a match for the TypeParameter provided.  On it's way also collects information
     * from any parameterized types and their super-types.
     *
    ...
    /**
     * Build a map of TypeVariables to Types.  We have to traverse the class hierarchy from subclass to superclass, but
     * the mapping of TypeVariables to Types has to start from the place were the type variable was originally defined
     * (superclass) and map to the place where the type is bound to an actual class (subclass).  We therefore need to
     * build up the mappings sub to super and then traverse super to sub.
     *
    ...
    /**
     * Gets the preferred bound from the WildcardType provided. In the case of '? super SomeClass' then 'SomeClass' will
     * be returned. In the case of '? extends AnotherClass' then 'AnotherClass' will be returned.
     *
    ...
    /**
     * Fetches the type of value that can be get/set with this expression evaluation. This is equivalent (though more
     * efficient) to calling getValue().getClass().  If the type information on this expression is not complete then
    ...
    /**
     * Returns a scalar type appropriate to the expression evaluation. When {@link #getType()} returns a scalar type
     * then this method will return the identical class. However, when getType() returns an Array, a Collection or a Map
     * this method will attempt to determine the type of element stored in that Array/Collection/Map and return that
     * Class.  If getType() returns null due to insufficient type information this method will also return null.
     * Similarly if the type of item in the Array/Collection/May cannot be determined then String.class will be
     * returned.
     *
     * @return The scalar type to which values should be converted in order to either be set using this expression or
     *         set into the Array/Collection/Map should this expression point at a non scalar property
     */
    ...
    /**
     * <p>Fetches the value of this expression evaluated against the bean. This is equivalent to calling the appropriate
     * get() methods in sequence in order to fetch the property indicated by the expression.</p>
     * <p/>
     * <p>If the property or any intermediate property in the expression is null this method will return null and will
     * not alter the state of the object graph.</p>
     *
    ...
    /**
     * <p>Sets the value of the expression evaluated against the bean. This is loosely equivalent to calling the
     * appropriate getter() on intermediate properties and then calling the appropriate setter on the final
     * sub-property.</p>
     * <p/>
     * <p>During set operations null intermediate nodes will be instantiated and linked into the object graph in order
     * to persistently set the desired property. When this is not possible (e.g. because of a lack of default
    ...
    /**
     * <p>Attempts to create a default value for a given node by either a) creating a new array instance for arrays, b)
     * fetching the first enum for enum classes, c) creating a default instance for interfaces and abstract classes
     * using ReflectUtil or d) calling a default constructor.
     *
    ...
    /**
     * <p>Sets the value of this expression to "null" for the bean.  In reality this is not always null, but the logical
     * interpretation of "null" for a given type. For primitives the value is set to be the default value for the
     * primitive type as used by the JVM when initializing instance variables.  For Map entries the key is removed from
     * the Map instead of leaving the key present with a null value.</p>
     * <p/>
     * <p>If any intermediate properties in the expression are null this method will return immediately. The sole
     * purpose of this method is to blank out a value <i>if one is present</i>. Therefore if no value is present,
